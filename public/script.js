class ZFrame {
    ...
    async handleLogin() {
        ...
        try {
            ...
            // For now, we'll just store the credentials and show authenticated state
            // In a real app, you would validate credentials with the server
            this.username = username;
            this.isAuthenticated = true;

            // Store credentials
            localStorage.setItem('zframe_username', username);
    ...
    async generateNewToken() {
        ...
        if (!this.username) {
            this.showMessage(messageContainer, 'Please login first', 'error');
            return;
        }

        // Since we don't have the password stored, we'll prompt for it
        const password = prompt('Please enter your password to generate a new token:');
        if (!password) return;

        try {
            this.showMessage(messageContainer, 'Generating token...', 'info');

            const response = await fetch(`${this.apiBase}/get_token`, {
                method: 'POST',
                headers: {
    ...
    async handleUpload() {
        ...
        if (!this.token) {
            this.showMessage(messageContainer, 'Please generate an API token first', 'error');
            return;
        }
        ...
        try {
            ...
            this.showMessage(messageContainer, 'Uploading image...', 'info');
            ...
            if (response.ok && data.filename) {
                this.showMessage(messageContainer, `Image uploaded successfully: ${data.filename}`, 'success');
                ...
                progressContainer.classList.add('hidden');
                this.showMessage(messageContainer, `Upload failed: ${data.error}`, 'error');
            }
        } catch (error) {
            console.error('Upload error:', error);
            progressContainer.classList.add('hidden');
            this.showMessage(messageContainer, 'Network error during upload. Please try again.', 'error');
        }
    }
    ...
    async loadUserStats() {
        try {
            // For now, we'll count the images we can load
    ...
    async loadUserImages() {
        ...
        try {
            ...
            // Try to use the API endpoint if we have a token, otherwise fall back to pattern loading
            if (this.token) {
                ...
                if (response.ok) {
                    ...
                    // If token fails, fall back to pattern loading
                    await this.loadImagesByPattern();
                }
            } else {
                // No token, use pattern loading to show all available images
                await this.loadImagesByPattern();
            }
    ...
    async loadImagesByPattern() {
        ...
        // Try to load images numbered 1-100
        for (let i = 1; i <= 100; i++) {
            const extensions = ['jpg', 'png', 'webp'];
            
            for (const ext of extensions) {
                try {
                    const imageUrl = `${this.apiBase}/images/${i}.${ext}`;
                    ...
                    if (response.ok) {
                        this.userImages.push({
                            ...
                            filename: `${i}.${ext}`,
                            url: `/images/${i}.${ext}`,
                            size: 0, // We don't have size info from HEAD request
                            uploadDate: new Date(), // We don't have real dates, so use current
                            extension: ext
                        });
                        break;
                    }
